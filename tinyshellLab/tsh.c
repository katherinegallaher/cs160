/* 
   Name: Katherine Gallaher 861100447
 ...
 *    it immediately.  
 * built in commands include: quit, jobs, bg <job>, fg <job>
 */
int builtin_cmd(char **argv) 
{
...
void do_bgfg(char **argv) 
{
	if(argv[1] == NULL){//invalid input
		printf("A pid or jid must be given\n");
		return;
	}
	struct job_t *job;
	char *id = argv[1];
	...
	//syntax is bg %number or fg %number if jid 
	//just number for pid????
	
	//get job from pid or jid, check for errors 
